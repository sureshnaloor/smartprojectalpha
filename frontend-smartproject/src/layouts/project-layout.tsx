...
export default function ProjectLayout({ children, projectId }: ProjectLayoutProps) {
  ...
  const isDocumentPage = location.includes('/under-construction/ProjectDrawings') ||
                         ...
                         location.includes('/under-construction/EquipmentDrawings') ||
  ...
  const getPageInfo = () => {
    ...
    if (location.includes('/under-construction/EquipmentDrawings')) {
      return { title: 'Equipment Drawings', icon: <FolderOpen className="h-4 w-4" /> };
    }
