...
import { Loader2, Link as LinkIcon, ExternalLink, Trash2, Mail } from "lucide-react";
...
export default function ProjectCorrespondence() {
    ...
    const createMutation = useMutation({
        mutationFn: async () => {
            const res = await apiRequest("POST", `/api/projects/${projectId}/correspondence/create`, {
                name,
                link,
                description,
        ...
        onSuccess: () => {
            ...
            toast({
                title: "Success",
                description: "Correspondence link added successfully",
            });
            setSubmitting(false);
            setName("");
            setLink("");
    ...
    const deleteMutation = useMutation({
        mutationFn: async ({ fileId, fileName }: { fileId: string; fileName: string }) => {
            ...
            const res = await apiRequest("DELETE", `/api/projects/${projectId}/correspondence?fileId=${fileId}&fileName=${encodedFileName}`);
            if (!res.ok) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {items?.length === 0 && (
                        <div className="col-span-full flex flex-col items-center justify-center p-12 text-muted-foreground border-2 border-dashed rounded-xl bg-muted/10">
                            ...
                            <p className="text-lg font-medium">No correspondence links added yet</p>
