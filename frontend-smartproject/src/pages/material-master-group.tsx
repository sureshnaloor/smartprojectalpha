...
async function createGroup(data: Omit<MaterialGroup, "id" | "createdAt" | "updatedAt">): Promise<MaterialGroup> {
    const response = await fetch("/api/material-groups", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
    });
    if (!response.ok) throw new Error("Failed to create material group");
    return response.json();
}

async function updateGroup(
    id: number,
    data: Partial<Omit<MaterialGroup, "id" | "createdAt" | "updatedAt">>
    ...
    if (!response.ok) throw new Error("Failed to update material group");
    return response.json();
}
...
async function deleteGroup(id: number): Promise<void> {
    ...
    if (!response.ok) throw new Error("Failed to delete material group");
}
...
export default function MaterialMasterGroup() {
    ...
    const createMutation = useMutation({
        ...
        onError: () => {
            toast({ title: "Error creating Material Group", variant: "destructive" });
        },
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: number; data: Partial<Omit<MaterialGroup, "id" | "createdAt" | "updatedAt">> }) =>
            updateGroup(id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/material-groups"] });
        ...
        onError: () => {
            toast({ title: "Error updating Material Group", variant: "destructive" });
        },
    });
    ...
    const deleteMutation = useMutation({
        ...
        onError: () => {
            toast({ title: "Error deleting Material Group", variant: "destructive" });
        },
    });
            ...
            <div className="p-8 min-h-screen" style={{
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="mb-8">
                        ...
                        <p className="text-gray-600">Manage Material Groups used in Material Master.</p>
                    </div>
