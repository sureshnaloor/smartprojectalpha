...
import { Loader2, Link as LinkIcon, ExternalLink, Trash2, HardHat } from "lucide-react";
...
export default function ProjectSubcontractCorrespondence() {
    ...
    const { data: items, isLoading } = useQuery<CorrespondenceItem[]>({
        queryKey: [`/api/projects/${projectId}/subcontract-correspondence`],
    });

    const createMutation = useMutation({
        mutationFn: async () => {
            const res = await apiRequest("POST", `/api/projects/${projectId}/subcontract-correspondence/create`, {
                name,
                link,
                description,
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: [`/api/projects/${projectId}/subcontract-correspondence`] });
            toast({
                title: "Success",
                description: "Subcontract correspondence link added successfully",
            });
            setSubmitting(false);
            setName("");
            setLink("");
    ...
    const deleteMutation = useMutation({
        mutationFn: async ({ fileId, fileName }: { fileId: string; fileName: string }) => {
            ...
            const res = await apiRequest("DELETE", `/api/projects/${projectId}/subcontract-correspondence?fileId=${fileId}&fileName=${encodedFileName}`);
            if (!res.ok) {
        ...
        <div className="space-y-8 p-6 bg-background min-h-screen">
            ...
            <div className="relative flex items-center justify-center mb-8 p-6 rounded-xl bg-card border shadow-[0_10px_20px_rgba(0,0,0,0.1),0_6px_6px_rgba(0,0,0,0.1)] transition-all hover:shadow-[0_14px_28px_rgba(0,0,0,0.15),0_10px_10px_rgba(0,0,0,0.12)]">
                <div className="text-center">
                    ...
                    <p className="text-muted-foreground mt-2">Track Important Subcontractor Emails and Links</p>
                </div>
            ...
            <Card className="border-2 border-dashed shadow-sm hover:border-primary/50 transition-colors">
                <CardHeader>
                    <CardTitle className="text-lg">Add New Subcontract Link</CardTitle>
                </CardHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {items?.length === 0 && (
                        <div className="col-span-full flex flex-col items-center justify-center p-12 text-muted-foreground border-2 border-dashed rounded-xl bg-muted/10">
                            <HardHat className="h-12 w-12 mb-4 opacity-50" />
                            <p className="text-lg font-medium">No subcontract correspondence links added yet</p>
